// import React from 'react';

const ServiceListing = () => {
	return (
		<>
			<div className='section pad-btm-sec services-listing-page'>
				<div className='container'>
					<h2 className='services-list'>
						{' '}
						496 results for <span>Health</span> near{' '}
						<span>Hyderabad</span>
					</h2>
					<div className='row no-gutters justify-content-between'>
						{/*Left Panel Start*/}
						<div className='left-panel'>
							<h4>Categories</h4>
							<div className='services-categories'>
								<div className='card'>
									<div>
										<div className='card-body'>
											<ul className='catgory-list'>
												<li>
													<a href='javascript:void(0)'>
														Acupuncture
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Ambulance Services
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Ayurvedic Medicines
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Ayurvedic Treatment
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Bangles
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Beauty &amp; Wellness
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Beauty Parlours
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Children&apos;s Hospitals
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Chit Funds
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Clinics
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Contact Lenses
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														CPAP &amp; BIPAP Systems
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Dental Clinics
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Diagnostic Centres
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Doctors
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Education Colleges
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Emergency Services
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														ENT Hospitals
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Eye Hospitals
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Eyeglasses
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Fertility &amp;
														Infertility Clinics
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Financial Services
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Fitness Centres
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														General Hospitals
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														General Pharmacies
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Hair Stylists
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Health Clubs
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Homeopathy Clinics
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Homeopathy Medicines
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Hospitals
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Lab Equipment And
														Chemical Suppliers
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Laboratories
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Ladies Dresses
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Maternity Hospitals
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Medical Equipments
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Medical Stockings
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Mobile Phones
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Multispecialty Hospitals
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Optics &amp; Eyewear
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Oxygen Concentrators
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Pharmaceutical Companies
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Pharmacies
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Physiotherapy Clinics
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Rehabilitation Centres
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Sarees &amp; Blouses
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Spa &amp; Saloon
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Super Specialty
														Hospitals
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Surgical Instruments
													</a>
												</li>
												<li>
													<a href='javascript:void(0)'>
														Unani Treatment
													</a>
												</li>
											</ul>
											<div className='viewmore-btn'>
												<a
													href='javascript:void(0)'
													className='btn  custom-btn'
												>
													View More
												</a>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
						{/*Left Panel End*/}
						{/*Right Panel Start*/}
						<div className='right-panel'>
							<div className='row'>
								<div className='col-md-12'>
									<div className='store-box '>
										<div className='store-box-pic'>
											<img
												src='/assets/images/store.jpg'
												alt
											/>
										</div>
										<div className='store-short-detail'>
											<h2>
												Sagar Homoe Stores{' '}
												<i className='fas fa-check-circle verified' />
											</h2>
											<div className='store-summry'>
												<div className='row'>
													<div className='col-lg-7 col-md-12 col-sm-6'>
														<div className='store-service'>
															<div className='store-deals'>
																Homeopathy
																Medicines,
																Pharmacies
															</div>
															<div className='store-timing'>
																<i className='far fa-clock' />{' '}
																<span>
																	Open
																</span>{' '}
																- until 8:00PM
															</div>
															<div className='store-website'>
																<i className='fas fa-globe' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Website
																</a>{' '}
															</div>
														</div>
													</div>
													<div className='col-lg-5 col-md-12 col-sm-6'>
														<div className='store-service-right'>
															<div className='phonenum'>
																{' '}
																<i className='fas fa-phone-alt' />{' '}
																<a href='tel:+914064525370'>
																	{' '}
																	+91 40
																	64525370
																</a>
															</div>
															<div className='address'>
																{' '}
																<i className='fas fa-location-arrow' />{' '}
																Abids Hyderabad
																- 500001
															</div>
															<div className='map-location'>
																<i className='fas fa-map-marker-alt' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Directions
																</a>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div className='col-md-12'>
									<div className='store-box '>
										<div className='store-box-pic'>
											<img
												src='/assets/images/store.jpg'
												alt
											/>
										</div>
										<div className='store-short-detail'>
											<h2>
												Sagar Homoe Stores{' '}
												<i className='fas fa-check-circle' />
											</h2>
											<div className='store-summry'>
												<div className='row'>
													<div className='col-lg-7 col-md-12 col-sm-6'>
														<div className='store-service'>
															<div className='store-deals'>
																Homeopathy
																Medicines,
																Pharmacies
															</div>
															<div className='store-timing'>
																<i className='far fa-clock' />{' '}
																<span>
																	Open
																</span>{' '}
																- until 8:00PM
															</div>
															<div className='store-website'>
																<i className='fas fa-globe' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Website
																</a>{' '}
															</div>
														</div>
													</div>
													<div className='col-lg-5 col-md-12 col-sm-6'>
														<div className='store-service-right'>
															<div className='phonenum'>
																{' '}
																<i className='fas fa-phone-alt' />{' '}
																<a href='tel:+914064525370'>
																	{' '}
																	+91 40
																	64525370
																</a>
															</div>
															<div className='address'>
																{' '}
																<i className='fas fa-location-arrow' />{' '}
																Abids Hyderabad
																- 500001
															</div>
															<div className='map-location'>
																<i className='fas fa-map-marker-alt' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Directions
																</a>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div className='col-md-12'>
									<div className='store-box '>
										<div className='store-box-pic'>
											<img
												src='/assets/images/store.jpg'
												alt
											/>
										</div>
										<div className='store-short-detail'>
											<h2>
												Sagar Homoe Stores{' '}
												<i className='fas fa-check-circle' />
											</h2>
											<div className='store-summry'>
												<div className='row'>
													<div className='col-lg-7 col-md-12 col-sm-6'>
														<div className='store-service'>
															<div className='store-deals'>
																Homeopathy
																Medicines,
																Pharmacies
															</div>
															<div className='store-timing'>
																<i className='far fa-clock' />{' '}
																<span>
																	Open
																</span>{' '}
																- until 8:00PM
															</div>
															<div className='store-website'>
																<i className='fas fa-globe' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Website
																</a>{' '}
															</div>
														</div>
													</div>
													<div className='col-lg-5 col-md-12 col-sm-6'>
														<div className='store-service-right'>
															<div className='phonenum'>
																{' '}
																<i className='fas fa-phone-alt' />{' '}
																<a href='tel:+914064525370'>
																	{' '}
																	+91 40
																	64525370
																</a>
															</div>
															<div className='address'>
																{' '}
																<i className='fas fa-location-arrow' />{' '}
																Abids Hyderabad
																- 500001
															</div>
															<div className='map-location'>
																<i className='fas fa-map-marker-alt' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Directions
																</a>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div className='col-md-12'>
									<div className='store-box '>
										<div className='store-box-pic'>
											<img
												src='/assets/images/store.jpg'
												alt
											/>
										</div>
										<div className='store-short-detail'>
											<h2>
												Sagar Homoe Stores{' '}
												<i className='fas fa-check-circle' />
											</h2>
											<div className='store-summry'>
												<div className='row'>
													<div className='col-lg-7 col-md-12 col-sm-6'>
														<div className='store-service'>
															<div className='store-deals'>
																Homeopathy
																Medicines,
																Pharmacies
															</div>
															<div className='store-timing'>
																<i className='far fa-clock' />{' '}
																<span>
																	Open
																</span>{' '}
																- until 8:00PM
															</div>
															<div className='store-website'>
																<i className='fas fa-globe' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Website
																</a>{' '}
															</div>
														</div>
													</div>
													<div className='col-lg-5 col-md-12 col-sm-6'>
														<div className='store-service-right'>
															<div className='phonenum'>
																{' '}
																<i className='fas fa-phone-alt' />{' '}
																<a href='tel:+914064525370'>
																	{' '}
																	+91 40
																	64525370
																</a>
															</div>
															<div className='address'>
																{' '}
																<i className='fas fa-location-arrow' />{' '}
																Abids Hyderabad
																- 500001
															</div>
															<div className='map-location'>
																<i className='fas fa-map-marker-alt' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Directions
																</a>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div className='col-md-12'>
									<div className='store-box '>
										<div className='store-box-pic'>
											<img
												src='/assets/images/store.jpg'
												alt
											/>
										</div>
										<div className='store-short-detail'>
											<h2>
												Sagar Homoe Stores{' '}
												<i className='fas fa-check-circle' />
											</h2>
											<div className='store-summry'>
												<div className='row'>
													<div className='col-lg-7 col-md-12 col-sm-6'>
														<div className='store-service'>
															<div className='store-deals'>
																Homeopathy
																Medicines,
																Pharmacies
															</div>
															<div className='store-timing'>
																<i className='far fa-clock' />{' '}
																<span>
																	Open
																</span>{' '}
																- until 8:00PM
															</div>
															<div className='store-website'>
																<i className='fas fa-globe' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Website
																</a>{' '}
															</div>
														</div>
													</div>
													<div className='col-lg-5 col-md-12 col-sm-6'>
														<div className='store-service-right'>
															<div className='phonenum'>
																{' '}
																<i className='fas fa-phone-alt' />{' '}
																<a href='tel:+914064525370'>
																	{' '}
																	+91 40
																	64525370
																</a>
															</div>
															<div className='address'>
																{' '}
																<i className='fas fa-location-arrow' />{' '}
																Abids Hyderabad
																- 500001
															</div>
															<div className='map-location'>
																<i className='fas fa-map-marker-alt' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Directions
																</a>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div className='col-md-12'>
									<div className='store-box '>
										<div className='store-box-pic'>
											<img
												src='/assets/images/store.jpg'
												alt
											/>
										</div>
										<div className='store-short-detail'>
											<h2>
												Sagar Homoe Stores{' '}
												<i className='fas fa-check-circle' />
											</h2>
											<div className='store-summry'>
												<div className='row'>
													<div className='col-lg-7 col-md-12 col-sm-6'>
														<div className='store-service'>
															<div className='store-deals'>
																Homeopathy
																Medicines,
																Pharmacies
															</div>
															<div className='store-timing'>
																<i className='far fa-clock' />{' '}
																<span>
																	Open
																</span>{' '}
																- until 8:00PM
															</div>
															<div className='store-website'>
																<i className='fas fa-globe' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Website
																</a>{' '}
															</div>
														</div>
													</div>
													<div className='col-lg-5 col-md-12 col-sm-6'>
														<div className='store-service-right'>
															<div className='phonenum'>
																{' '}
																<i className='fas fa-phone-alt' />{' '}
																<a href='tel:+914064525370'>
																	{' '}
																	+91 40
																	64525370
																</a>
															</div>
															<div className='address'>
																{' '}
																<i className='fas fa-location-arrow' />{' '}
																Abids Hyderabad
																- 500001
															</div>
															<div className='map-location'>
																<i className='fas fa-map-marker-alt' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Directions
																</a>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div className='col-md-12'>
									<div className='store-box '>
										<div className='store-box-pic'>
											<img
												src='/assets/images/store.jpg'
												alt
											/>
										</div>
										<div className='store-short-detail'>
											<h2>
												Sagar Homoe Stores{' '}
												<i className='fas fa-check-circle' />
											</h2>
											<div className='store-summry'>
												<div className='row'>
													<div className='col-lg-7 col-md-12 col-sm-6'>
														<div className='store-service'>
															<div className='store-deals'>
																Homeopathy
																Medicines,
																Pharmacies
															</div>
															<div className='store-timing'>
																<i className='far fa-clock' />{' '}
																<span>
																	Open
																</span>{' '}
																- until 8:00PM
															</div>
															<div className='store-website'>
																<i className='fas fa-globe' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Website
																</a>{' '}
															</div>
														</div>
													</div>
													<div className='col-lg-5 col-md-12 col-sm-6'>
														<div className='store-service-right'>
															<div className='phonenum'>
																{' '}
																<i className='fas fa-phone-alt' />{' '}
																<a href='tel:+914064525370'>
																	{' '}
																	+91 40
																	64525370
																</a>
															</div>
															<div className='address'>
																{' '}
																<i className='fas fa-location-arrow' />{' '}
																Abids Hyderabad
																- 500001
															</div>
															<div className='map-location'>
																<i className='fas fa-map-marker-alt' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Directions
																</a>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div className='col-md-12'>
									<div className='store-box '>
										<div className='store-box-pic'>
											<img
												src='/assets/images/store.jpg'
												alt
											/>
										</div>
										<div className='store-short-detail'>
											<h2>
												Sagar Homoe Stores{' '}
												<i className='fas fa-check-circle' />
											</h2>
											<div className='store-summry'>
												<div className='row'>
													<div className='col-lg-7 col-md-12 col-sm-6'>
														<div className='store-service'>
															<div className='store-deals'>
																Homeopathy
																Medicines,
																Pharmacies
															</div>
															<div className='store-timing'>
																<i className='far fa-clock' />{' '}
																<span>
																	Open
																</span>{' '}
																- until 8:00PM
															</div>
															<div className='store-website'>
																<i className='fas fa-globe' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Website
																</a>{' '}
															</div>
														</div>
													</div>
													<div className='col-lg-5 col-md-12 col-sm-6'>
														<div className='store-service-right'>
															<div className='phonenum'>
																{' '}
																<i className='fas fa-phone-alt' />{' '}
																<a href='tel:+914064525370'>
																	{' '}
																	+91 40
																	64525370
																</a>
															</div>
															<div className='address'>
																{' '}
																<i className='fas fa-location-arrow' />{' '}
																Abids Hyderabad
																- 500001
															</div>
															<div className='map-location'>
																<i className='fas fa-map-marker-alt' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Directions
																</a>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div className='col-md-12'>
									<div className='store-box '>
										<div className='store-box-pic'>
											<img
												src='/assets/images/store.jpg'
												alt
											/>
										</div>
										<div className='store-short-detail'>
											<h2>
												Sagar Homoe Stores{' '}
												<i className='fas fa-check-circle' />
											</h2>
											<div className='store-summry'>
												<div className='row'>
													<div className='col-lg-7 col-md-12 col-sm-6'>
														<div className='store-service'>
															<div className='store-deals'>
																Homeopathy
																Medicines,
																Pharmacies
															</div>
															<div className='store-timing'>
																<i className='far fa-clock' />{' '}
																<span>
																	Open
																</span>{' '}
																- until 8:00PM
															</div>
															<div className='store-website'>
																<i className='fas fa-globe' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Website
																</a>{' '}
															</div>
														</div>
													</div>
													<div className='col-lg-5 col-md-12 col-sm-6'>
														<div className='store-service-right'>
															<div className='phonenum'>
																{' '}
																<i className='fas fa-phone-alt' />{' '}
																<a href='tel:+914064525370'>
																	{' '}
																	+91 40
																	64525370
																</a>
															</div>
															<div className='address'>
																{' '}
																<i className='fas fa-location-arrow' />{' '}
																Abids Hyderabad
																- 500001
															</div>
															<div className='map-location'>
																<i className='fas fa-map-marker-alt' />{' '}
																<a href='javascript:void(0)'>
																	{' '}
																	Directions
																</a>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
							<nav>
								<ul className='pagination'>
									<li className='page-item disabled'>
										<a
											className='page-link'
											href='#'
											aria-label='Previous'
										>
											<i className='fas fa-chevron-left' />
										</a>
									</li>
									<li className='page-item active'>
										<a className='page-link' href='#'>
											1
										</a>
									</li>
									<li className='page-item'>
										<a className='page-link' href='#'>
											2
										</a>
									</li>
									<li className='page-item'>
										<a className='page-link' href='#'>
											3
										</a>
									</li>
									<li className='page-item'>
										<a
											className='page-link'
											href='#'
											aria-label='Next'
										>
											<i className='fas fa-chevron-right' />
										</a>
									</li>
								</ul>
							</nav>
						</div>
						{/*Right Panel end*/}
					</div>
				</div>
			</div>
		</>
	);
};

export default ServiceListing;
